...
import { createPlaceholderEmbedding, initializeTensorFlow } from "./tensorflow-fallback"
...
class SentimentAnalyzer {
  analyzeSentiment(text: string): {
    ...
    const subjectivity =
      subjectivityIndicators.length > 0
        ? Math.min(
            subjectivityIndicators.reduce((sum, score) => sum + score, 0) / words.length,
            1.0,
          )
        : 0
...
export class SemanticAnalyzer {
  ...
  async initialize(): Promise<void> {
    ...
    try {
      // Initialize TensorFlow backend if available
      const tfInitialized = await initializeTensorFlow()
      if (!tfInitialized) {
        console.warn("TensorFlow not available, semantic analysis will be limited")
      ...
      // Continue without semantic analysis instead of throwing
      this.initialized = false
    }
  }
  ...
  async generateEmbedding(
    ...
    if (!isBrowser) {
      return {
        slug,
        embedding: createPlaceholderEmbedding(512), // Placeholder embedding
    ...
    if (!this.model) {
      console.warn("Semantic model not available, returning placeholder embedding")
      return {
        slug,
        embedding: createPlaceholderEmbedding(512), // Placeholder embedding
        content: combinedText,
  ...
  calculateSentimentAwareSimilarity(
    ...
    const { sentimentWeight = 0.3, preferSameEmotion = true, polarityTolerance = 1.0 } = options
    ...
    const sentimentAlignment = this.sentimentAnalyzer.calculateSentimentAlignment(
      source.sentiment,
      target.sentiment,
    )
  ...
  calculateCrossReferenceStrength(
    ...
    if (source.sentiment && target.sentiment) {
      const alignment = this.sentimentAnalyzer.calculateSentimentAlignment(
        source.sentiment,
        target.sentiment,
      )
