...
export async function handleBuild(argv) {
  ...
  if (argv.serve) {
    ...
    const server = http.createServer(async (req, res) => {
      ...
      // handle root path - serve the-rats-garden-of-wisdom.html
      if (fp === "/") {
        const indexFp = "the-rats-garden-of-wisdom.html"
        if (fs.existsSync(path.posix.join(argv.output, indexFp))) {
          req.url = "/the-rats-garden-of-wisdom.html"
          return serve()
        }
      }
      ...
      if (fp.endsWith("/")) {
        // /trailing/
        // does /trailing/the-rats-garden-of-wisdom.html exist? if so, serve it
        const indexFp = path.posix.join(fp, "the-rats-garden-of-wisdom.html")
        if (fs.existsSync(path.posix.join(argv.output, indexFp))) {
          req.url = fp
          return serve()
        }
        ...
        // does /regular/the-rats-garden-of-wisdom.html exist? if so, redirect to /regular/
        let indexFp = path.posix.join(fp, "the-rats-garden-of-wisdom.html")
        if (fs.existsSync(path.posix.join(argv.output, indexFp))) {
          return redirect(fp + "/")
        }
      }
