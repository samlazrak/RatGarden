...
module.exports = __toCommonJS(ai_assistant_exports);
...
var handler = async (event, context) => {
  ...
  try {
    ...
    switch (selectedProvider) {
      case "gemini":
        aiResponse = await callGeminiAPI(text, feature, context2);
        break;
      case "openai":
        aiResponse = await callOpenAIAPI(text, feature);
        break;
      case "anthropic":
        aiResponse = await callAnthropicAPI(text, feature, context2);
        break;
      default:
        return {
          statusCode: 400,
...
function getOptimalAnthropicModel(feature, textLength) {
  ...
  if (textLength > 2e3) return "claude-3-5-sonnet-20241022";
